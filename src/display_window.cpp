        ...
        m_should_quit(false)
{
    m_quad_vs_code =
        ...
        "varying vec2 tex_coord_through;"
    ...
    m_quad_fs_code =
        ...
        "void main(void)"
        "{"
        "   gl_FragColor = texture2D(tex_sampler, tex_coord_through);"
        "}";
...
void display_window::key_event(int sdl_key, bool press)
{
    if (m_key_event_func != nullptr)
        m_key_event_func(sdl_key, press);
}
...
void update(void* arg) 
{
    ...
    display_window* display = reinterpret_cast<display_window*>(arg);
    display->poll_events();
    display->render();
...
//    emscripten_set_main_loop_arg(update, reinterpret_cast<void*>(this), 0, 0);
#endif
    
    std::cout << "past main loop!" << std::endl;
